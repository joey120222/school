<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Passport.aspx.cs" Inherits="SchoolInfo.Admin.Passport" %>
        ...
        <tbody>
            <%
                for (int i = 0; i < dtDataList.Rows.Count; i++)
                {
                    ...
                    <tr>
                        <td><%= i + 1 %></td>
                        <td><%= dtDataList.Rows[i]["passport_type"]%></td>
                        <td><%= dtDataList.Rows[i]["student_name"]%></td>
                        <td><%= dtDataList.Rows[i]["student_code"]%></td>
                        <td><%= Convert.ToDateTime(dtDataList.Rows[i]["passport_date"]).ToString("yyyy-MM-dd HH:mm:ss")%></td>
                        <td><a href="javascript:previewOpen('<%= dtDataList.Rows[i]["passport_image"]%>')"><%= dtDataList.Rows[i]["passport_image"]%></a></td>
                        <td><%= dtDataList.Rows[i]["passport_content"]%></td>
                    </tr>
                    <%
                }
            %>
